...
$NXT_ELECT_1 * * *    cd ${SCRIPT_DIR} && ./dtik_depool.sh >> /var/ton-work/validator_msig.log
...
CRONT_JOBS=$(cat <<-_ENDCRN_
$NXT_ELECT_1 * * *    script --return --quiet --append --command "cd ${SCRIPT_DIR} && ./dtik_depool.sh >> /var/ton-work/validator_msig.log"
